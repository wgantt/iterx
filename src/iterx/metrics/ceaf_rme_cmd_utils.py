...
from iterx.metrics.muc.ceaf_rme import (
    ScoreFunction,
    SCORER_CONSTRUCTOR,
    IterXTemplate,
    generate_scoring_structures,
)
...
def _load_predictions(
    ...
    if file_type == PredictionFileType.IterX:
        return jsonlines_to_gtt_templates(
            str(pred_file),
            False,
            False,
        ...
        k: [
            {
                ...
                else [list(set([m for c in sv for m in c]))]
                for s, sv in t.items()
            }
            for t in v
        ]
...
def load_references(
    ref_file: Path, dataset: DatasetKind, loader_configs: Dict[str, Any]
) -> OrderedDict:
    if dataset == DatasetKind.MUC:
        return read_gold_templates(file_path=str(ref_file), **loader_configs)
...
def print_prediction_comparison(
    ...
    for doc_id in sorted((set(golds.keys()) | set(preds.keys()))):
        ...
        p_num, p_den, r_num, r_den, scores, _, _, (row, col) = scorer.metric(
            pred, gold, output_raw=True
        )
        ...
        print(
            f"doc_id={doc_id}\t#pred={len(pred)}\t#gold={len(gold)}\tprec={prec:.4f}\trec={recall:.4f}\tf1={f1:.4f}"
        )
        ...
        for row_idx, col_idx in zip_longest(row, col, fillvalue=-1):
            if row_idx == -1:
                ...
                common_keys = set(preds[doc_id][col_idx].keys()) & set(
                    golds[doc_id][row_idx].keys()
                )
                common_keys.remove("incident_type")
                print(
                    f'\tincident_type=\t{preds[doc_id][col_idx]["incident_type"]}'
                ...
                for key in common_keys:
                    ...
                    for pred_val, gold_val in zip_longest(
                        sorted(
                            preds[doc_id][col_idx][key],
                            key=lambda x: -max([len(y) for y in x]),
                        ),
                        sorted(
                            golds[doc_id][row_idx][key],
                            key=lambda x: -max([len(y) for y in x]),
                        ),
                    ):
